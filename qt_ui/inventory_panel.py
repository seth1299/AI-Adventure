...
class InventoryPanel(QWidget):
    ...
    def _format_table_html(self, grid_text: str) -> str:
        """
        Wraps the tabulated text in an HTML <pre> block with protective styling.
        
        This prevents PySide6's rich text engine from clipping the top or bottom 
        borders of box-drawing characters (like those used in 'rounded_grid').
        
        Args:
            grid_text (str): The raw text output generated by the tabulate library.
            
        Returns:
            str: A formatted HTML string safe for rendering in a QTextBrowser.
        """
        try:
            # We maintain line-height: 1.0 so vertical borders connect perfectly,
            # but add padding and a leading newline so the top boundary isn't shaved off.
            formatted_html = (
            ...
            logging.error(f"InventoryPanel: Failed to format table HTML. Returning raw grid. Details: {error}")
            return f"\n{grid_text}\n\n"
